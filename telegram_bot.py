...
async def notify_drivers_group(context, order_code, details, price):
    ...
    for g in groups:
        try:
            await context.bot.send_message(chat_id=g["chat_id"], text=text, reply_markup=InlineKeyboardMarkup(kb))
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("accept_"):
        ...
        if not ok:
            ...
            return
        kb = [[InlineKeyboardButton(core.get_msg("button_finish"), callback_data=f"finish_{order_code}")]]
        await query.edit_message_text(
            f"طلب #{order_code} — تم القبول بواسطة {driver['name']}\nرقم العميل: {customer['phone']}",
    ...
    try:
        ...
        if state == "awaiting_pickup":
            ...
            order_code = core.create_order(customer["id"], order_type, customer["draft_vehicle"],
                                            customer["draft_details"], price, "telegram")
            ...
            await notify_drivers_group(context, order_code, customer["draft_details"], price)
